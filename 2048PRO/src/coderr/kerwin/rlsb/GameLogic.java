...
import coderr.kerwin.rlsb.model.AnimationGrid;
import coderr.kerwin.rlsb.model.BaseCell;
import coderr.kerwin.rlsb.model.Grid;
...
public class GameLogic {
    ...
    public GameLogic(Context context, GameView view) {
        mContext = context;
        mView = view;
        endingMaxValue = (int) Math.pow(2, 18 - 1);
    }
    ...
    public void newGame() {
        ...
        mView.prevMaxValue = 0;
        mView.currMaxValue = 0;
        mView.resyncTime();
        mView.invalidate();
    }
    ...
    private void addRandomTile() {
        if (grid.isCellsAvailable()) {
            ...
            TileCell tile = new TileCell(grid.randomAvailableCell(), value);
            spawnTile(tile);
        }
    }

    private void spawnTile(TileCell tile) {
    ...
    private void prepareTiles() {
        for (TileCell[] array : grid.field) {
            for (TileCell tile : array) {
    ...
    private void moveTile(TileCell tile, BaseCell cell) {
    ...
    public void move(int direction) {
        ...
        prepareUndoState();
        BaseCell vector = getVector(direction);
        ...
        for (int xx: traversalsX) {
            for (int yy: traversalsY) {
                BaseCell cell = new BaseCell(xx, yy);
                TileCell tile = grid.getCellContent(cell);

                if (tile != null) {
                    BaseCell[] positions = findFarthestPosition(cell, vector);
                    TileCell next = grid.getCellContent(positions[1]);

                    if (next != null && next.getValue() == tile.getValue() && next.getMergedFrom() == null) {
                        TileCell merged = new TileCell(positions[1], tile.getValue() * 2);
                        TileCell[] temp = {tile, next};
    ...
    private BaseCell getVector(int direction) {
        BaseCell[] map = {
                new BaseCell(0, -1), // up
                new BaseCell(1, 0),  // right
                new BaseCell(0, 1),  // down
                new BaseCell(-1, 0)  // left
        };
        return map[direction];
    }

    private List<Integer> buildTraversalsX(BaseCell vector) {
    ...
    private List<Integer> buildTraversalsY(BaseCell vector) {
        List<Integer> traversals = new ArrayList<Integer>();
    ...
    private BaseCell[] findFarthestPosition(BaseCell cell, BaseCell vector) {
        BaseCell previous;
        BaseCell nextCell = new BaseCell(cell.getX(), cell.getY());
        do {
            previous = nextCell;
            nextCell = new BaseCell(previous.getX() + vector.getX(),
                    previous.getY() + vector.getY());
        } while (grid.isCellWithinBounds(nextCell) && grid.isCellAvailable(nextCell));

        BaseCell[] answer = {previous, nextCell};
        return answer;
    }
    ...
    private boolean tileMatchesAvailable() {
        ...
        for (int xx = 0; xx < numSquaresX; xx++) {
            for (int yy = 0; yy < numSquaresY; yy++) {
                tile = grid.getCellContent(new BaseCell(xx, yy));

                if (tile != null) {
                    for (int direction = 0; direction < 4; direction++) {
                        BaseCell vector = getVector(direction);
                        BaseCell cell = new BaseCell(xx + vector.getX(), yy + vector.getY());

                        TileCell other = grid.getCellContent(cell);
    ...
    private boolean positionsEqual(BaseCell first, BaseCell second) {
